...
class VideoflowUserManager {
    ...
    $fbuser_details = $this->getFBuserInfo($fbuser);
    ...
    if (JVERS3) {
    $query = 'SELECT * FROM #__users WHERE name LIKE '.$db->quote ('%'.$db->escape ($fullname, true).'%',false );
    }
    $db->setQuery($query);
    $jname = $db->loadObject();
    if ($jname) {
      ...
      if (JVERS3) {
      $query = 'SELECT * FROM #__users WHERE name LIKE '.$db->quote ('%'.$db->escape ($xname, true).'%',false );
      }
      $db->setQuery($query);
      $nname = $db->loadObject();
      if ($nname) $fullname = $name.'_'.$rname; else $fullname = $xname; 
    }
    if (empty($fbuser_details['username'])) {
    $username = $name;
    } else {
    ...
    if (JVERS3) {
    $query = 'SELECT * FROM #__users WHERE username LIKE '.$db->quote ('%'.$db->escape ($username, true).'%',false );	
    }
    $db->setQuery($query);
    $jusername = $db->loadObject();
    if (!empty($jusername)) $username = $rname;
    if (empty($fbuser_details['email'])){
        $url = JURI::root(); 
    ...
    {
          $db = JFactory::getDBO();
          $query = 'SELECT * FROM #__vflow_users WHERE fb_id ='.$fbuser;
          $db->setQuery( $query );
