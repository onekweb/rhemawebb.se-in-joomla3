...
class VideoflowCategoryManager {
   ...
   function newCat ($catname){
    $catid = '';
    $db = JFactory::getDBO ();
    $query = 'SELECT * FROM #__vflow_categories WHERE name = '.$db->quote ($db->escape ($catname, true), false );
    $db->setQuery($query);
    $cat = $db->loadObject();
    $catid = $cat->id;
    if (!$catid) {
  ...
  function getCategories(){     
    ...
    $clist->date = $date->toFormat('%Y-%m-%d');
    } else {
    $clist->date = $date->format('%Y-%m-%d');  
    }
    $carray[0] = $clist;
    }
    return $carray;
  ...
  $db = JFactory::getDBO();
  $query = 'SELECT id from #__vflow_categories WHERE name='.$db->quote ($db->escape ($name, true), false );
  $db->setQuery( $query );
  $id = $db->loadResult();
  if (empty($id)) $id = '';
  return $id;
