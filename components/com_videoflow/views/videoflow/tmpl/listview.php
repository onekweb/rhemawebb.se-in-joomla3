                  ...
                  <table width="100%" cellspacing="0" cellpadding="0" border="0" valign="top">
                    <tbody>
                      <tr><td>
                          ...
                          echo '<a href="'.$fbuserdata['link'].'"><img src="http://graph.facebook.com/'.$fbuser.'/picture" alt="'.$fbuserdata['name'].'"/></a>';     
                          echo '</div>';     
                          echo '<div style="position:relative; margin-top:20px; float:right;">';
                          echo '<a href="'.$fbuserdata['link'].'">'.$fbuserdata['first_name'].'</a>';
                          echo '</div>';     
                          }
                          if ($fbuser || $this->vflow1) echo '</div>'; //End top banner division
                          ?>
                          <div id="vmess" style="position:fixed; left: 50%; top: 10%; width: 600px; margin-left: -300px; z-index: 100;"></div>
                    </td></tr>
                  </tbody>
                </table>
               <!-- TOP SECTION -->
...
if ($vtask == 'search') {
    ...
    if (!empty ($this->vlist)) {
    $search_message = JText::_('COM_VIDEOFLOW_RESULTS_FOR').' <b>'.$searchword.'</b>:';
    } 
    if (empty($this->vlist) && (!empty ($searchword))){
    $search_message = JText::_('COM_VIDEOFLOW_NO_RESULTS').' <b>'.$searchword.'</b>.'.JText::_('COM_VIDEOFLOW_TRY_NEW_TERM'); 
    }
    if ($vs == 'rel' ) {
    $reltitle = JRequest::getString('title');
    $search_message = JText::_('COM_VIDEOFLOW_RELATED_TO').' <b>'.$reltitle.'</b>:';
    }
...
$stask = 'visit&pid='.$pid;
$vuser = JRequest::getString('usrname', JText::_('COM_VIDEOFLOW_GUEST'));
echo '<div id="vfsearch_msg">'.JText::_('COM_VIDEOFLOW_MEDIA_FROM').' <b>'.$vuser.'</b>:</div>';
}
...
$vuser = JRequest::getString('usrname', JText::_('COM_VIDEOFLOW_GUEST'));
echo '<div id="vfsearch_msg">'.JText::_('COM_VIDEOFLOW_LIKED_BY').' <b>'.$vuser.'</b>:</div>';
}
// Display "videos in category" if necessary
if ($vtask == 'cmed'){
$cname = JRequest::getString('cname');
echo '<div id="vfsearch_msg">'.JText::_('COM_VIDEOFLOW_MEDIA_CAT').' <b>'.$cname.'</b>:</div>';
}
// Flat category view
if ($vtask == 'cats'){
echo '<div id="vfsearch_msg">'.JText::_('COM_VIDEOFLOW_MEDIA_CAT').' <b>'.$this->catname.'</b>:</div>';
}
...
if (!empty($this->vlist) && is_array ($this->vlist)){
  ...
  foreach ($this->vlist as $vid){
      ...
      $vid->rating = JText::_('COM_VIDEOFLOW_NONE');
      }
                ...
                <td class="vflist10" width="20%" valign="top">
                  <table class="vfinfobar">
                    <tbody>
                      ...
                      <tr><td>
                          <font class="vflist6">
                            <?php echo JText::_('COM_VIDEOFLOW_INFO_COMMENTS'); ?>
                          </font>
                          <font class="vflist5">
                      ...
                      <tr>
                        <td valign="middle" height="15">
                          <font class="vflist6">
                            <?php echo JText::_('COM_VIDEOFLOW_INFO_RATING'); ?>
                          </font>
                          <font class="vflist5">
                            <?php echo $vid->rating; ?> 
                          </font></td>
